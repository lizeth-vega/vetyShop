...
export const NewProduct = () => {
    ...
    const [inventario, setInventario] = useState(0) //arraca en cero xq es numerico
    const [vendedor, setVendedor] = useState("")
    const [imagen, setImagen] = useState([])
    const [imagenPreview, setImagenPreview] = useState([])

    //arreglo con las opciones que deberian aparecer en el formulario
    const categorias = [
        "Alimento seco",
        "Alimento humedo",
        "Accesorios",
    ...
    //mensajes de posble error o exito
    useEffect(() => {
        ...
        if (success) {
            ...
            dispatch({ type: NEW_PRODUCT_RESET }) //limpia la pantalla
        }
    ...
    const submitHandler = (e) => {
        ...
        const formData = new FormData(); //setea toda la informacion
    ...
    const onChange = e => {
        ...
        files.forEach(file => {
            ...
            reader.onload = () => {
                if (reader.readyState === 2) { // si uno es vacio 2 lleno
                    //genere los datos del form data la url 
